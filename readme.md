...
After a release PR merge, `release` can publish changed packages and create package tags and GitHub releases through the GitHub API:

```bash
npx packtory release --publish --tag --push --github-release --no-dry-run
```

`release` reads `GH_TOKEN` when set, otherwise `GITHUB_TOKEN`. Tags and GitHub releases are created remotely through the GitHub API; the command does not commit changelog files or run local Git tag and push commands.
...
1. **Automatic Versioning (Default) 🔄:**
   ...
   - Compare the contents of all files from the downloaded tarball with the contents of all files resolved from the bundler:
     ...
     - If there are any differences, increase the latest version number by one (patch version), generate a new `package.json`, verify that the exact target version is not already published with different artifacts, smoke-import public runtime exports from the generated package layout, create a tarball, and publish the new version.
...
2. **Manual Versioning:**
   ...
   - Before publishing, Packtory verifies the exact chosen version, then smoke-imports public runtime exports from changed generated artifacts. A version that already exists with different artifacts, or exists without being tagged `latest`, fails before any publish write starts.
...
4. **`changelog`** (Optional, Object):
   - Configures changelog generation and outputs for `packtory changelog` and `packtory release-pr maintain`.
...
6. Prunes pure runtime code files that have no surviving behavior after DCE and are no longer reached by surviving imports. Public roots, explicit resources, generated manifests, declarations, assets, side-effecting files, and files still needed for module evaluation are preserved.
7. Recomputes dependency metadata from the transformed and pruned code. External packages and sibling bundle dependencies that only appeared in removed code are no longer emitted into `dependencies`, `peerDependencies`, check inputs, or vendored package roots.
