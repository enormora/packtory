...
suite('checks-schema', function () {
    suite('areTheTypesWrong check schema', function () {
        ...
        test('top-level schema accepts a global allowList on noDuplicatedFiles', function () {
            assert.strictEqual(
                safeParse(checksSchema, { noDuplicatedFiles: { enabled: true, allowList: [ 'src/index.ts' ] } })
                    .success,
                true
            );
        });
    ...
    suite('noDuplicatedFiles top-level schema', function () {
    ...
    suite('noDuplicatedFiles per-package schema', function () {
        ...
        test('per-package schema accepts an allowList of file paths', function () {
            assert.strictEqual(
                safeParse(checksPerPackageSchema, { noDuplicatedFiles: { allowList: [ 'src/shared/util.ts' ] } })
                    .success,
                true
            );
        });
        ...
        test('per-package schema rejects an enabled flag', function () {
            assert.strictEqual(
                safeParse(checksPerPackageSchema, { noDuplicatedFiles: { enabled: true } }).success,
                false
            );
        });
    ...
    suite('requiredFiles check schema', function () {
        test('top-level schema rejects an empty path in requiredFiles.files', function () {
            assert.strictEqual(
                safeParse(checksSchema, { requiredFiles: { enabled: true, files: [ '' ] } }).success,
                false
            );
        });
        ...
        test('per-package schema accepts requiredFiles with a files list', function () {
            assert.strictEqual(
                safeParse(checksPerPackageSchema, { requiredFiles: { files: [ 'LICENSE' ] } }).success,
                true
            );
        });
        ...
        test('top-level schema accepts maxBundleSize with enabled and a bytes threshold', function () {
            assert.strictEqual(
                safeParse(checksSchema, { maxBundleSize: { enabled: true, bytes: 1024 } }).success,
                true
            );
        });
        ...
        test('top-level schema rejects a non-integer byte threshold', function () {
            assert.strictEqual(
                safeParse(checksSchema, { maxBundleSize: { enabled: true, bytes: 1.5 } }).success,
                false
            );
        });
    ...
    suite('maxBundleSize and unused dependency checks', function () {
        ...
        test('top-level schema accepts noUnusedBundleDependencies with enabled', function () {
            assert.strictEqual(
                safeParse(checksSchema, { noUnusedBundleDependencies: { enabled: true } }).success,
                true
            );
        });
    ...
    suite('dev dependency and target path checks', function () {
        ...
        test('per-package schema rejects any field on uniqueTargetPaths', function () {
            assert.strictEqual(
                safeParse(checksPerPackageSchema, { uniqueTargetPaths: { enabled: true } }).success,
                false
            );
        });
    });
