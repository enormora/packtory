...
function packageWithoutSources(name: string, jsFile: string): PacktoryConfigWithoutRegistry['packages'][number] {
    return {
        name,
        mainPackageJson: { type: 'module' },
...
function packagesWithoutSources(): readonly PacktoryConfigWithoutRegistry['packages'][number][] {
    return [ packageWithoutSources('pkg-a', 'a.js'), packageWithoutSources('pkg-b', 'b.js') ] as const;
}

suite('changelog-settings', function () {
    suite('changelog schema outputs and labels', function () {
    ...
    suite('changelog output validation', function () {
        ...
        test('validation rejects target-scoped label patterns without the target placeholder', function () {
            assert.deepStrictEqual(
                validateChangelogSettings(configWith({ targetScopedLabelPattern: 'scope:{label}' })),
                [
