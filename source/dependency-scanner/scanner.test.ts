...
test('analyzes the given entryPoint file in the given folder as a typescript project', async () => {
    ...
    assert.deepStrictEqual(analyzeProject.firstCall.args, [ '/foo', {failOnCompileErrors: false, moduleResolution: 'module', resolveDeclarationFiles: false} ]);
});
...
test('passes the failOnCompileErrors option to the project analyzer', async () => {
    ...
    assert.strictEqual(analyzeProject.callCount, 1);
    assert.deepStrictEqual(analyzeProject.firstCall.args, [ '/foo', {failOnCompileErrors: true, moduleResolution: 'module', resolveDeclarationFiles: false} ]);
});
...
test('passes the resolveDeclarationFiles option to the project analyzer', async () => {
    ...
    assert.strictEqual(analyzeProject.callCount, 1);
    assert.deepStrictEqual(analyzeProject.firstCall.args, [ '/foo', {failOnCompileErrors: false, moduleResolution: 'module', resolveDeclarationFiles: true} ]);
});
...
test('passes the moduleResolution option to the project analyzer', async () => {
    ...
    assert.strictEqual(analyzeProject.callCount, 1);
    assert.deepStrictEqual(analyzeProject.firstCall.args, [ '/foo', {failOnCompileErrors: false, moduleResolution: 'common-js', resolveDeclarationFiles: false} ]);
});
...
test('returns no dependencies if the given file doesn’t have any dependencies', async () => {
    ...
    assert.deepStrictEqual(result, {localFiles: [ {filePath: '/dir/entry.js', substitutionContent: Maybe.nothing()} ], topLevelDependencies: {}});
});
...
test('tries to locate soure map files for all local files when includeSourceMapFiles is true', async () => {
    const locate = fake.resolves(Maybe.nothing());
    const getReferencedSourceFilePaths = stub().onFirstCall().returns([ '/dir/foo.js' ]).onSecondCall().returns([ '/dir/bar.js' ]).onThirdCall().returns([])
    const analyzeProject = createFakeAnalyzeProject({getReferencedSourceFilePaths});
...
test('returns the local dependency files found in subsequent dependencies', async () => {
    const getReferencedSourceFilePaths = stub().onFirstCall().returns([ '/dir/foo.js', '/dir/bar.js' ]).onSecondCall().returns([]).onThirdCall().returns([ '/dir/baz.js' ]).onCall(3).returns([]);
    const analyzeProject = createFakeAnalyzeProject({getReferencedSourceFilePaths});
    const dependencyScanner = dependencyScannerFactory({analyzeProject});
...
test('doesn’t include any files from node_modules in localFiles', async () => {
    ...
    assert.deepStrictEqual(result.localFiles, [ {filePath: '/dir/entry.js', substitutionContent: Maybe.nothing()}, {filePath: '/dir/foo.js', substitutionContent: Maybe.nothing()} ]);
});
...
test('throws an error when an invalid node_modules path is returned', async () => {
    ...
    try {
        ...
        assert.strictEqual((error as Error).message, 'Couldn’t find node_modules package name for \'/invald/node_modules/\'');
    }
});
