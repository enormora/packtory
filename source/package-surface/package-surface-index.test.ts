...
import { indexPublicModules, summarizePackageSurface } from './package-surface-index.ts';

const binsOnlyExplicitBundle = {
    name: 'package-a',
...
suite('package-surface-index', function () {
    ...
    test('summarizePackageSurface rejects explicit surfaces without public entries', function () {
        ...
        assert.throws(() => {
            summarizePackageSurface({
                ...
                surface: {
                    mode: 'explicit',
                    packageInterface: invalidPackageInterface as never
                }
            });
    ...
    test('indexPublicModules rejects unsupported surface modes', function () {
        ...
        assert.throws(() => {
            indexPublicModules(invalidBundle as unknown as Parameters<typeof indexPublicModules>[0]);
        }, /^Error: Unsupported package surface mode: mystery$/u);
    });
});
