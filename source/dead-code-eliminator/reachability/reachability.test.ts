...
import { bfsClosure } from './bfs-closure.ts';
...
suite('reachability', function () {
    ...
    suite('external seeds and traversal state', function () {
        ...
        test('expandWith returns local reachability unchanged when the local and external seeds are empty', function () {
            const index = buildReachabilityIndex({ files: [], entryPointFilePaths: new Set<string>() });
        ...
        test('bfsClosure returns the initial visited set when no seed is available', function () {
            const visited = new Set([ 'already-visited' ]);
            const reachable = bfsClosure(
                [],
                function () {
                    throw new Error('empty seed traversal should not expand');
                },
                visited,
                {
                    maximumNodeCount: 0,
                    dependencies: {
                        visitedHas(candidateVisited, value) {
                            return candidateVisited.has(value);
                        }
                    }
                }
            );

            assert.deepStrictEqual(reachable, visited);
        });
