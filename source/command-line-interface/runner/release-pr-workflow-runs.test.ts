...
function assertWorkflowIdentifierMatches(): void {
...
suite('release-pr-workflow-runs', function () {
    suite('workflow selection', function () {
        test('matches workflows by id, name, path, and basename', function () {
            assertWorkflowIdentifierMatches();
        });
    ...
    suite('workflow run lookup', function () {
        ...
        test('accepts workflow dispatch runs with null or exact matching workflow paths', function () {
            assert.strictEqual(
                findWorkflowRunIdInRuns(
                    [
                        {
