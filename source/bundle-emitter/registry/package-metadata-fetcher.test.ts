...
suite('package-metadata-fetcher', function () {
    ...
    suite('package tarballs', function () {
        ...
        test('fetchPackageTarball accepts tarballs exactly at the download limit', async function () {
            const buffer = fake.resolves(Buffer.from('tarball-bytes'));
            const getHeader = fake.returns('268435456');
            ...
            ) as unknown as NpmFetch;

            const result = await fetchPackageTarball(npmFetch, tarballUrl, settings);

            assert.deepStrictEqual(result, Buffer.from('tarball-bytes'));
            assert.deepStrictEqual(getHeader.firstCall.args, [ 'content-length' ]);
        });

        test('fetchPackageTarball rejects tarballs above the download limit', async function () {
            const buffer = fake.resolves(Buffer.from('unused'));
            const npmFetch = Object.assign(
                fake.resolves({
                    buffer,
                    headers: { get: fake.returns('268435457') }
                }),
                {
                    json: Object.assign(fake(), { stream: fake() }),
                    pickRegistry: fake()
            ...
            try {
                await fetchPackageTarball(npmFetch, tarballUrl, settings);
