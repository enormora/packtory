...
const missingFileExpectedMessage = 'Provenance bundle file "/build/pkg.sigstore" does not exist.' +
...
suite('publish-settings-bridge', function () {
    suite('publish options', function () {
    ...
    suite('auto provenance error remaps', function () {
        ...
        test('remapPublishError() rewrites the missing GitLab SIGSTORE_ID_TOKEN error', function () {
            expectRemapped(
                Object.assign(
                    new Error(
                        'Provenance generation in GitLab CI requires "SIGSTORE_ID_TOKEN" with "sigstore" audience'
                    ),
                    { code: 'EUSAGE' }
                ),
                { access: 'public', provenance: { type: 'auto' } },
    ...
    suite('file provenance error remaps', function () {
    ...
    suite('unmapped errors', function () {
