...
function bundleWithExplicitBin(
    contents: readonly AnalyzedBundleResource[],
    binField: NonNullable<VersionedBundleWithManifest['binField']>
): VersionedBundleWithManifest {
    return versionedBundleWithManifest({
        contents,
        packageJson: { name: 'the-name', version: 'the-version' },
...
type CapturedArtifactEntries<TProjected> = {
    readonly entries: readonly TProjected[];
};

type ArtifactsBuilderFixture = {
    readonly builder: ArtifactsBuilder;
    readonly fileManager: FakeFileManager;
};

type ArtifactsBuilderBroadcasterFixture = {
    readonly builder: ArtifactsBuilder;
...
type Overrides = {
    readonly fileManager?: FakeFileManager;
    readonly tarballBuilder?: Partial<BuildStub>;
    readonly zipBuilder?: Partial<BuildStub>;
};

function createBuildStub(overrides: Partial<BuildStub> = {}): BuildStub {
    return {
        build: overrides.build ?? fake.resolves(Buffer.from([ -1 ]))
    };
}

function artifactsBuilderFactory(overrides: Overrides = {}): ArtifactsBuilderFixture {
...
function builderWithUnreadableTargetFolder(): ArtifactsBuilderFixture {
...
suite('artifacts-builder', function () {
    suite('base operations', function () {
        ...
        test('buildFolder() throws when the target folder already exists', async function () {
            ...
            try {
                ...
                assert.deepStrictEqual(fileManager.getCheckReadabilityCall(0), {
                    fileOrFolderPath: '/the/target/folder'
                });
    ...
    suite('archive contents', function () {
    ...
    suite('progress reporting', function () {
        ...
        ): readonly CapturedArtifactEntries<TProjected>[] {
            const received: { readonly entries: readonly TProjected[]; }[] = [];
            broadcaster.consumer.on('artifactsCollected', function (payload) {
                received.push({ entries: payload.entries.map(project) });
            });
            return received;
        }

        function artifactsBuilderWithBroadcaster(): ArtifactsBuilderBroadcasterFixture {
        ...
        test('collectContents() emits artifact entries with size and kind classification', function () {
            const { builder, broadcaster } = artifactsBuilderWithBroadcaster();
            const received = captureArtifactEntries(broadcaster, function (entry) {
                return {
                    path: entry.path,
        ...
        test('collectContents() emits exactly once per invocation', function () {
            const { builder, broadcaster } = artifactsBuilderWithBroadcaster();
            let callCount = 0;
            broadcaster.consumer.on('artifactsCollected', function () {
                callCount += 1;
            });
