...
function visitGraphBreadthFirstSearch<TId extends GraphNodeId, TData>(
    ...
    const completed = Array.from({ length: nodes.size + countEdges(nodes) + 1 }).some(function () {
        if (head === undefined) {
            return true;
        }
...
type TopologicalDiscoveryStep<TId extends GraphNodeId> = {
...
function advanceOrCompleteTopologicalDiscovery<TId extends GraphNodeId, TData>(
    nodes: ReadonlyMap<TId, GraphNode<TId, TData>>,
    dependencies: GraphDependencies<TId>,
    discovery: TopologicalDiscovery<TId>
): TopologicalDiscoveryStep<TId> {
    ...
    if (currentGeneration.length === 0) {
        return { generations: discovery.generations };
    }
    return { discovery: advanceTopologicalDiscovery(nodes, dependencies, discovery, currentGeneration) };
}

function topologicalDiscoveryAttempts<TId extends GraphNodeId, TData>(
    count: number,
    nodes: ReadonlyMap<TId, GraphNode<TId, TData>>,
    dependencies: GraphDependencies<TId>
): readonly ((discovery: TopologicalDiscovery<TId>) => TopologicalDiscoveryStep<TId>)[] {
    return Array.from({ length: count }, function () {
        return function (discovery) {
            return advanceOrCompleteTopologicalDiscovery(nodes, dependencies, discovery);
        };
    });
}

function isCompleteTopologicalDiscovery<TId extends GraphNodeId>(
    step: TopologicalDiscoveryStep<TId>
): step is { readonly generations: readonly (readonly TId[])[]; } {
    return Object.hasOwn(step, 'generations');
}
...
function collectTopologicalGenerations<TId extends GraphNodeId, TData>(
    ...
    const exhaustedAttempts = nodes.size + 1;
    const attempts = topologicalDiscoveryAttempts(exhaustedAttempts, nodes, dependencies);

    for (const attempt of attempts) {
        const step = attempt(discovery);
        if (isCompleteTopologicalDiscovery(step)) {
