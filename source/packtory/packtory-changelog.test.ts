...
type EngineCalls = {
    ...
    readonly resolvePullRequestLabels: SinonSpy<
        readonly [ResolvePullRequestLabelsOptions],
...
function registerDependencyUpdateTests(): void {
    ...
    test('includes manifest dependency pull requests whose title mentions a changed dependency', async function () {
        const { engine, calls } = createEngine();
        const pullRequests: readonly PullRequest[] = [
            { id: 3, title: 'Update Alpha to 2.0.0' },
            { id: 4, title: 'Update Alpha in source only' },
            { id: 5, title: 'Update Beta only' },
            { id: 6, title: 'Update Alpha without changed files' },
            { id: 7, title: 'Update unrelated dependency' }
        ];
        const customEngine = {
            ...engine,
            collectMergedPullRequests: fake.resolves(pullRequests),
            ...
            readPullRequestChangedFiles: fake.resolves(
                new Map([
                    [ 3, [ 'package-lock.json' ] ],
                    [ 4, [ 'source/index.ts' ] ],
                    [ 5, [ 'package-lock.json', 'source/index.ts' ] ],
                    [ 7, [ 'package-lock.json' ] ]
                ])
            )
        } as unknown as PrLogEngine;

        await render([ releasePackage({ changelogDependencyNames: [ 'alpha', 'beta' ] }) ], customEngine);

        assert.deepStrictEqual(calls.resolvePullRequestLabels.firstCall.args[0].pullRequests, [
            { id: 3, title: 'Update Alpha to 2.0.0' },
            { id: 5, title: 'Update Beta only' }
        ]);
    });
