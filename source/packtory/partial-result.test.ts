...
suite('partial-result', function () {
    ...
    test('succeededResultsFrom ignores function failures with partial-shaped properties', function () {
        function failure(): void {
            return undefined;
        }
        Object.assign(failure, { succeeded: [ 'wrong' ], failures: [] });

        assert.deepStrictEqual(succeededResultsFrom(Result.err(failure)), []);
    });
