...
async function runSinglePackageFailure(
    scheduler: SchedulerType
): ReturnType<SchedulerType['runForEachScheduledPackage']> {
    return scheduler.runForEachScheduledPackage({
        ...
        async execute() {
            throw new Error('package-a failed');
        },
...
suite('scheduler', function () {
    ...
    test('runForEachScheduledPackage() returns no succeeded results when the only package fails', async function () {
        const { scheduler } = createTestScheduler();

        const result = await runSinglePackageFailure(scheduler);

        const error = getErrResult(result, 'Expected result to be an error');
        assert.deepStrictEqual(error.succeeded, []);
        assert.deepStrictEqual(
            error.failures.map(function (failure) {
