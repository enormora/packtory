...
suite('package-processor publish', function () {
    ...
    test('buildAndPublish() rejects non-ESM package manifests', async function () {
        const { processor } = createProcessor();
        const buildOptions: BuildAndPublishOptions = {
            ...createBuildAndPublishOptions(),
            mainPackageJson: { type: 'commonjs' } as unknown as BuildAndPublishOptions['mainPackageJson']
        };

        await assert.rejects(
            processor.buildAndPublish({
                analyzedBundle: createAnalyzedBundle(),
                buildOptions,
                stage: false
            }),
