...
function registerPlanningTests(): void {
    ...
    test('plans changed package manifest dependency names from previous and current artifacts', async function () {
        const result = await planFor({
            packageNames: [ 'pkg-a' ],
            buildResults: [
                buildResultFor({
                        ...
                        files: [
                            {
                                ...
                                content: JSON.stringify({ dependencies: { alpha: '1.0.0', beta: '1.0.0' } }),
                                isExecutable: false
                            }
                        ]
                    })
            ...
            collectContents() {
                return [
                    {
                        filePath: 'package/package.json',
                        content: JSON.stringify({ dependencies: { alpha: '2.0.0', gamma: '1.0.0' } }),
                        isExecutable: false
                    }
                ];
            }
        ...
        assert.deepStrictEqual(expectFirstPackage(result).changelogDependencyNames, [ 'alpha', 'beta', 'gamma' ]);
    });

    test('omits changed package manifest dependency names when the current manifest is absent', async function () {
        const result = await planFor({
            packageNames: [ 'pkg-a' ],
            buildResults: [
                buildResultFor({
                    previousReleaseArtifacts: previousReleaseArtifactsFor({
                        ...
                        files: [
                            {
                                filePath: 'package/package.json',
                                content: JSON.stringify({ dependencies: { alpha: '1.0.0' } }),
                                isExecutable: false
                            }
                        ]
                    })
