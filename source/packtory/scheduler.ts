...
function fulfilledPackageSuccess<TResult, TOptions>(
    result: PromiseSettledResult<PackageSuccess<TResult, TOptions>>
): result is PromiseFulfilledResult<PackageSuccess<TResult, TOptions>> {
    return result.status === 'fulfilled';
}

function rejectedPackageSuccess<TResult, TOptions>(
    result: PromiseSettledResult<PackageSuccess<TResult, TOptions>>
...
function collectPackageSuccesses<TResult, TOptions>(
    ...
    const succeeded = results
        .filter(fulfilledPackageSuccess)
        .map(function (result) {
            return result.value;
        });
    ...
    return {
        succeeded,
        succeededResults: succeeded.map(function (success) {
            return success.result;
        }),
        failures: results
            .filter(rejectedPackageSuccess)
            .map(function (result) {
